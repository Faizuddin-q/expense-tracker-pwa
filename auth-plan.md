...
- **Update: dedicated signup flow (section 9 below).** Originally login and registration were combined into one form/endpoint (typing a never-seen phone number just registered it). Replaced with an explicit Sign in / Create account toggle and a separate `POST /api/auth/register` endpoint, for two reasons: (1) no "confirm password" field existed on the combined form, so a typo during first-time signup could lock someone out of an account with zero data and no recovery but an admin reset; (2) an explicit flow is clearer about what's actually happening. `/api/auth/login` no longer silently registers unknown numbers — it 404s and tells the client to sign up instead.
...
### 9. Dedicated signup flow
- [x] `MIN_PASSWORD_LENGTH` / `MAX_PASSWORD_LENGTH` / `isValidPhone` centralized in `lib/auth.ts` (previously duplicated across login and change-password)
- [x] New `POST /api/auth/register` — rejects if a `users` doc already exists (409, "sign in instead") or a legacy `profiles` doc already exists (409, same phone-as-password hint as login); otherwise creates the account, `passwordIsDefault: false`, sets the session cookie. Rate-limited 8/15min per IP+phone, same as login.
- [x] `POST /api/auth/login` tightened — an unknown phone number (no `users` doc, no `profiles` doc) now 404s with "Create one instead" rather than silently registering it. The legacy phone-as-password migration path is unchanged.
- [x] `lib/app-context.tsx` — `continueWithPhone` renamed to `signIn` (login only now); added `createAccount` calling `/api/auth/register`
- [x] `components/Login.tsx` — Sign in / Create account segmented toggle; signup mode adds a "Confirm password" field with mismatch styling and disables submit until they match (closes the typo-lockout gap); the "phone number is a temporary password" notice only shows in sign-in mode, since it doesn't apply to signup
- [x] `app/login/page.tsx` — owns `mode` and `confirmPassword` as local UI state, wires both submit paths
- [x] `npx tsc --noEmit` clean
- [x] Live-tested against the real DB with throwaway numbers (cleaned up after): register new number ✓, re-registering the same number → 409 ✓, logging into a freshly-registered account via `/api/auth/login` ✓, logging into a number with no account at all → 404 (not silently registered) ✓, registering a number that already has a legacy pre-password profile → 409 rather than overwriting it ✓
