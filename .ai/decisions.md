...
### Context
Built an entire new feature (Chapters) end to end, then iterated on it and on unrelated app-wide polish across several follow-up asks in the same session. Three commits landed on branch `feat/chapters` (off `dev`):

1. `1cf900f` - Add Chapters: track trips, purchases, and other one-off spends separately
2. `b739553` - Reuse real Expense components in Chapters; load Inter via `<head>` link
3. `a0dc20d` - Redesign Chapters list, fix mobile back nav, drop em dashes project-wide

### 1. Chapters feature - requirements gathered via structured Q&A before any code

**Why asked instead of assumed**: user's opening ask was vague ("add a feature to track a trip like Goa"), so requirements were gathered through several rounds of `AskUserQuestion` rather than guessed. Key answers that shaped the design:
- Not trip-specific - generalized to any one-off spend (a trip, a car purchase, anything), landing on the name "Chapters" (user picked it explicitly, over "Pockets"/"Milestones"/"Ledgers", with a note it may rename to "Pockets" later - left as a one-line comment on the nav entry, not implemented).
- Own dedicated page, not folded into the existing category picker.
- Tracked **separately** from regular monthly/overall totals - this became the core architectural constraint (see below).
- Any number of chapters, each with an optional budget settable any time (not required at creation).
- Solo only, no splitting/shared-expense tracking.

**Architecture decision - fully separate data path, not a tagged subset of `Expense`**: new Mongo collections (`chapters`, `chapterEntries`), new types (`types/chapter.ts`), new Zustand stores (`lib/chapter-store.ts`, `lib/chapter-entry-store.ts`), new API routes (`app/api/chapters/route.ts`, `app/api/chapters/entries/route.ts`), all deliberately independent of `useExpenses`/`useMonthSpend`/`useByCategory`. This was the only way to satisfy "kept separate from your monthly totals" - a `chapterId` tag on the existing `Expense` type would have required every existing total/aggregate to explicitly filter it out, an easy place to regress later. Full CRUD/sync pattern (soft-delete via `deletedAt`, `localId`-keyed upserts, bulk `POST`/`DELETE`) was copied structurally from the existing `expenses`/`categories` routes so it fits the app's established sync model rather than inventing a second one - down to reusing `lib/api/handler.ts`'s `withUserAuth` wrapper and the same rate-limit shape.

**Sync integration**: `lib/sync-store.ts`'s `runSync`/`SyncOptions`/`mergeSyncOptions` extended with `chapters`/`deletedChapterIds`/`chapterEntries`/`deletedChapterEntryIds` fields, pushed/pulled in the same `Promise.all` batch as expenses/categories/profile - one sync call still covers everything, no second sync pipeline. Chapter/entry stores get `resetOnLogout()` wired into `auth-store.ts`'s `clearSessionState`, same as every other store.

**Cascade delete on the backend**: `DELETE /api/chapters` also soft-deletes every `chapterEntries` row with a matching `chapterId` in the same request, so deleting a chapter doesn't orphan its entries server-side.

**Demo data**: `scripts/seed-demo-chapters.mjs` added (idempotent, upserts by `localId`, follows the existing `scripts/seed-demo-account.mjs` convention of `node --env-file=.env scripts/...`) after the user asked to seed mock chapters for the demo account (`1234567890`) directly in a running conversation, iterated twice more ("add about 8 more", "keep budgets between 20k and 4 lakh") before being asked to move the throwaway scratch scripts into the repo instead of a temp file - consolidated into one canonical script covering all 12 seeded chapters (67 expenses total) rather than leaving three separate one-off scripts behind.

### 2. Component reuse - asked directly by the user, not offered proactively

**What happened**: the first cut of the Chapters detail page (expense list, edit/delete dialogs, category breakdown) was built as parallel, hand-copied components (`ChapterEntryFormDialog`, `ChapterEntryDeleteDialog`, an inline `EntryRow`) rather than reusing the app's existing `ExpenseRow`/`ExpenseEditDialog`/`ExpenseDeleteDialog`/`expense-table.tsx`. The user asked directly: "why not use the existing components I have?" - the honest answer given was that those components are hard-typed to `Expense` and built around month-cycle filtering that doesn't apply to a chapter, but that the dialog markup and category-breakdown layout genuinely were duplicated. Then asked to "at least copy those components correctly," which was read as: don't approximate, genuinely reuse.

**Fix - generics, not a fork**: `ExpenseRow`, `ExpenseEditDialog`, `ExpenseDeleteDialog`, and `expense-table.tsx`'s `ExpenseTableRow`/`ExpenseListRow`/`ExpenseListRowData` were all made generic over a minimal `ExpenseLike` shape (`{category, note?, amount, date?, paymentMethod?, ...}`) instead of hard-typed to `Expense`, with the type parameter defaulting to `Expense` so every existing call site (`Home.tsx`, `Expenses.tsx`, `ExpenseDayDialog.tsx`) needed zero changes - confirmed via `tsc --noEmit` and a full build after. `ChapterDetail.tsx` now renders the exact same sortable, all-columns table as `/expenses` (via the genericized `ExpenseTableHeader`/`ExpenseTableRow`), the same `ExpenseEditDialog`/`ExpenseDeleteDialog` for edit/delete, and the same `Bar`-based row layout as `MonthlySummary`'s category breakdown instead of a plain list.

**What stayed separate, and why**: adding a new entry still needed its own dialog, since there's no existing reusable generic "add" dialog to copy from in the main app (Home adds expenses via inline composer controls, not a dialog). Rather than inventing a new pattern, followed the codebase's own existing precedent - `AdminAddExpenseDialog.tsx`'s comment "Same shell as ExpenseEditDialog, but seeded blank" - and built `ChapterAddEntryDialog.tsx` the same way. The old `ChapterEntryFormDialog`/`ChapterEntryDeleteDialog` were deleted once the real components covered edit/delete.

### 3. Bug caught after shipping: infinite render loop in the Chapters list

**Symptom** (reported by the user via a pasted Next.js error overlay): `"The result of getSnapshot should be cached to avoid an infinite loop"` plus `"Maximum update depth exceeded"`, both pointing at `ChapterCard` in `components/views/Chapters.tsx`.

**Root cause**: `useChapterEntries((s) => s.entries.filter((e) => e.chapterId === chapter.id))` - the selector's `.filter()` returns a **new array reference every render**, so Zustand's `useSyncExternalStore`-based snapshot comparison always sees a "changed" value and re-renders forever.

**Fix**: swapped it for the already-existing `useChapterEntriesFor(chapterId)` selector in `lib/chapter-entry-store.ts`, which wraps the same filter in `useMemo` - this selector already existed for exactly this case (it's what `ChapterDetail.tsx` uses) but the list page's card component had been written with an inline selector instead of reusing it. The page-level `allEntries = useChapterEntries((s) => s.entries)` selector elsewhere in the same file was already safe, since it returns the store's array directly rather than a derived one - worth remembering as the actual rule: a Zustand selector may return a stored reference directly, or a memoized derivation, but never a fresh array/object literal computed inline.

### 4. Mobile navigation reworked - bottom bar trimmed, "More" tab added, back-button logic iterated four times

**Ask**: mobile bottom nav should show only Add/Overview/Expenses + a "More" tab; tapping More opens a page listing the rest (Summary, Chapters, Settings).

**Implementation**: `lib/constants.ts` gained `mobileNavItems`/`moreNavItem`/`secondaryNavItems`, all derived from one `MOBILE_PRIMARY_IDS` array so the split stays in one place. New `/more` page (`app/(app)/more/page.tsx` + `components/views/More.tsx`) lists `secondaryNavItems` as tappable rows. `NavButton` gained an optional `forceActive` prop so the More tab can show active for any page nested under it, not just an exact pathname match. Desktop sidebar untouched - still renders the full `navItems` list, no "More" concept there.

**A live edit collision, handled by asking rather than overwriting**: while implementing, `lib/constants.ts`'s `MOBILE_PRIMARY_IDS` line changed on disk mid-edit, twice, in ways that contradicted the user's own stated request (first to include `summary`+`chapters` in the primary tabs, contracting what should be behind More). This turned out to be the user editing that exact line live in their own editor at the same time. Rather than repeatedly overwriting a line the user was visibly mid-edit on, stopped touching it and asked directly - user said they'd finish it themselves. Everything downstream (`mobileNavItems`, `secondaryNavItems`, the tab bar, the More page, both loading skeletons) is derived from that array, so whatever value they land on flows through automatically without needing another pass. **Lesson**: a file-changed-since-you-read-it notice on a line that contradicts the user's own just-stated instruction is a strong signal of a live concurrent edit, not a stale read - stop and ask instead of "fixing" it back.

**Back-button behavior on nested pages iterated four times, each time based on a screenshot or pasted symptom**:
1. First pass: added a back arrow in the header (`app/(app)/layout.tsx`) for any page under a "More" section, `router.push('/more')`, plus `ChapterDetail.tsx` kept its own separate in-page "← Chapters" link (pointing at `/chapters`) built at the same time.
2. User: "keep only back arrow chapters" - the header's condition (`pathname === item.href || pathname.startsWith(item.href + '/')`) was matching the nested `/chapters/[chapterId]` route too, showing the header's generic arrow *and* the page's own "← Chapters" link stacked. Narrowed the header condition to exact top-level matches only, so nested detail pages relied solely on their own in-page link.
3. User: "instead of Pockett it should be back arrow chapters, and clicking the arrow or the name should go to the previous page" - the header's title was falling back to the literal string `'Pockett'` on nested routes with no exact `PAGE_TITLES` entry. Reworked to compute a `moreSection` (the `secondaryNavItems` entry the current path is nested under, matched via `startsWith` again but now only for the *title fallback*, not the back-arrow visibility condition) so the title resolves to the parent section's label (`Chapters`/`Summary`/`Settings`) on any nested page. Also merged the icon and label into one clickable `<button>` (was icon-only next to inert text) and switched the click handler from a hardcoded `router.push('/more')` to `router.back()`, so it returns to wherever the user actually came from.
4. User pasted a screenshot showing two "← Chapters" rows stacked again (header's new back button, directly above `ChapterDetail.tsx`'s own one) - the in-page link in `ChapterDetail.tsx` was never removed in step 2, since step 2 only stopped the header from showing a *second* arrow, it didn't touch the page's own. Deleted `ChapterDetail.tsx`'s in-page back button entirely; the header now owns 100% of back navigation on mobile, and desktop is unaffected since it never showed either of these (sidebar handles nav there) and the header's own back button is `lg:hidden`.

**Takeaway for next time a nested-page back-button ask comes in**: check both the shared header (`app/(app)/layout.tsx`) and the page component itself for their own independent back-navigation UI before assuming one fix covers it - this feature had back-navigation logic living in two different files that had to be reconciled across three separate follow-ups.

### 5. Font: several rounds of "just try this," each reverted or replaced by the next ask

Sequence, in order, each fully superseding the last:
1. Simplified the font stack's fallback chain from `'Inter', system-ui, -apple-system, sans-serif` to `'Inter', sans-serif` (still via the existing Google Fonts `@import` in `app/globals.css`).
2. User asked why `IBM Plex Mono` (the separate `--font-mono` token, used only for `.font-mono-numbers`/`.tnum` tabular figures) was still showing - explained it's a distinct token from `--font-sans`, untouched by the fallback-chain ask.
3. "Use only Inter, no fallback anywhere" + "try Merriweather for mono" - swapped `--font-mono` to `'Merriweather', serif` briefly, flagged to the user that Merriweather doesn't actually support `tabular-nums`/`tnum` (a monospace-digit OpenType feature most serif text fonts lack), so numeric columns would likely stop aligning - reverted one message later when the user said "nahhh, replace mono with Inter only." Final state: both `--font-sans` and `--font-mono` are `'Inter', sans-serif`, no other family referenced anywhere (verified via a full-repo grep for `sans-serif`/`system-ui`/any other family name).
4. "Download Inter correctly from Google, use it only, no fallback" - downloaded the actual `latin`+`latin-ext` variable-font `.woff2` files from `fonts.gstatic.com` (not re-hosted from a third party), added local `@font-face` rules in `app/globals.css`, removed the `@import`. Verified against a running dev server: compiled CSS had zero `fonts.googleapis`/`fonts.gstatic` references, both local `.woff2` files served `200 font/woff2`. Used both subsets specifically because `latin-ext`'s Unicode range (`U+20AD-20C0`) is what covers the ₹ rupee sign the app's money formatting depends on - `latin` alone does not include it.
5. User asked why not just use the Google Fonts URL directly - answered with the actual trade-off (self-hosted: zero external request, pinned version; CDN link: simpler, auto-updated, one more third-party round-trip) and asked which they wanted. User: "use direct google link." Reverted the self-hosted files (`public/fonts/` deleted).
6. Final ask: "import it in the index html or from Next.js" (i.e., not a CSS `@import`, which blocks on the CSS bundle parsing before the browser even discovers the font request). Moved to `<link rel="preconnect">` + `<link rel="stylesheet">` tags directly in `app/layout.tsx`'s `<head>`, matching the pattern already used by a sibling project (`arrow-webapp`) the user referenced. Verified against the running dev server that Next.js also auto-added its own `<link rel="preload" as="style">` on top of the two manual tags.

**Net result, if a future session touches fonts again**: Inter only, loaded via `<link>` tags in `app/layout.tsx`'s `<head>` (not a CSS `@import`, not self-hosted `.woff2` files), no fallback family declared anywhere in `app/globals.css`.

### 6. A tangent that turned into real debugging: why does Inter's "1" not have a foot/serif here, but does in a sibling project

**What happened**: user asked why Inter's digit "1" doesn't render with a small base serif/foot the way it does in their `arrow-webapp` project, while using `tabular-nums`. First response was a general explanation (Inter's default "1" is a plain stroke; a footed variant exists behind a specific `cv0x` OpenType character-variant feature) plus a comparison HTML page published as an Artifact so the user could check visually without needing a local browser-automation setup (Playwright was explicitly declined earlier in the session).

**Investigation correction, twice**: first checked the wrong sibling repo (`verification-webapp`) and found it uses "Open Sans"/Proxima, not Inter at all - a plausible-looking but wrong answer, since it explained the visual difference (different typeface) without confirming it was actually the project the user meant. User corrected: "arrow-webapp, not verification-webapp, and use its dev branch with the latest pull." Re-ran `git fetch`/`checkout dev`/`pull` (65 commits behind, fast-forwarded) in the *correct* project directory and grepped its actual font setup: it does use Inter (`fontFamily: "Inter, system-ui, -apple-system, sans-serif"` in `theme.ts`), loaded via the same Google Fonts CSS2 endpoint, but always pairs `"tnum" 1` with `"lnum" 1"` (with a code comment: "Inter's default figures are proportional; tnum must be requested explicitly") - a real, verifiable difference from this project's plain `tabular-nums`, surfaced honestly rather than guessed at, including flagging that the actual glyph-shape cause couldn't be fully confirmed without rendering both side by side.

**Lesson**: when a user names a specific sibling project to check against, verify you're in the literal directory/repo they named (not a similarly-purposed one) before drawing conclusions - a wrong-but-plausible answer from the wrong repo is worse than saying "let me check the right one" a message later.

### 7. Chapters list page (`components/views/Chapters.tsx`) redesigned via the project's `impeccable` skill

**Ask**: "update UI/UX of the /chapters page more" - routed through the `impeccable` skill (`polish` playbook, since this was a narrow refinement of an existing page, not new-work) rather than done freehand, per the skill's own routing rules.

**Changes made, each tied to a specific drift/gap the skill's polish checklist surfaces**:
- Added a stat strip (chapter count / total tracked / expenses logged) at the top - the page previously had zero page-level summary, unlike `Dashboard.tsx`'s established stat-tile pattern.
- Replaced a hand-rolled progress-bar `<div>` with the shared `Bar` component - a "one-off implementation where a shared component already owns the pattern," per the skill's own drift taxonomy.
- Gave each card a colored icon badge (accent color derived from the chapter's id via the same hashing `getCategoryColor` already uses for categories - no new color-assignment logic invented) plus expense count and relative last-updated time (`formatRelativeTime`, already used in the admin table but not here).
- Archived chapters now render muted (reduced opacity) and sit behind a closed-by-default disclosure instead of always rendering in full color - reused the same expand/collapse chevron idiom `MonthlySummary`'s month rows already use, rather than inventing a new one.
- Fixed the header row's "New chapter" button silently stretching full-width on mobile with left-aligned content - an accidental side effect of `flex-col`'s default `align-items: stretch`, not a deliberate mobile CTA. Made it a deliberate full-width, centered button on mobile instead of leaving the accident in place.
- Ran the skill's mechanical detector (`detect.mjs`) after - zero findings. Did **not** do a browser screenshot pass, since Playwright was unavailable per the user's standing instruction from earlier in the session - flagged this gap explicitly rather than claiming a visual check that didn't happen.

### 8. Copy: every em dash replaced with a regular hyphen, project-wide

**Ask**: "don't use em dash anywhere in this project, use a normal dash."

**Scope decision**: excluded vendored skill tooling (`.claude/skills/*`, `.agents/skills/*`) and `node_modules` - those aren't this project's own code. Excluded `AGENTS.md` specifically, per its own embedded instruction that it's rewritten by `next dev` on every run and editing it manually is a no-op that just recreates the same uncommitted diff - **then included it anyway** in the final commit once its own text was re-read and found to say the opposite: "committing it with your work keeps the tree clean." (Corrected mid-task before the commit, not after.)

**Mechanical approach**: grepped for `—` across matching extensions (`.ts`/`.tsx`/`.md`/`.mjs`/`.css`), previewed a sample of matches to confirm every occurrence was the spaced `" — "` form (none were mid-word), then bulk-replaced via `sed`. Two residual categories needed a second pass after the first bulk replace: (a) em dashes sitting at the end of a line with the sentence continuing on the next line (`sed`'s ` — ` pattern doesn't match when the trailing space is actually a newline) - fixed with a `" —$"` end-of-line pattern; (b) em dashes used as a standalone placeholder glyph for "no value" (e.g. `'—'` returned when a number is missing/zero, in `lib/utils.ts`'s `moneyCompact`, `expense-table.tsx`'s date formatters, admin stat tiles) - these aren't prose at all, but the ask was read literally ("anywhere in this project") and replaced too. Also caught three non-`.ts`/`.md` files the initial extension filter missed (`public/manifest.json` - including the actual app display name shown on an installed home-screen icon, `public/sw.js`, `.github/workflows/bump-version.yml`, `pnpm-workspace.yaml`) via a follow-up grep with no extension filter.

**Verified**: `tsc --noEmit` clean, `next build` clean, `public/manifest.json` re-parsed as valid JSON after the sed pass (a blind global replace risked breaking JSON string escaping, checked explicitly rather than assumed), full test suite green.
